...
export const Content = () => {
  ...
  const handleSubmit = async (promptText: string) => {
    ...
    //  TODO: change the way we handle the diagram creation.
    if (!session) {
      router.push(
        `/auth/login?redirect=/diagram/${diagramId}?prompt=${encodeURIComponent(
          promptText
        )}`
      );
      return;
    ...
    router.push(
      `/diagram/${diagramId}?prompt=${encodeURIComponent(promptText)}`
    );
  };
